...
from model import TrackNet, InpaintNet, KeypointNet
...
def get_model(model_name, seq_len=None, bg_mode=None):
        ...
        Args:
            model_name (str): type of model to create
                Choices:
                    ...
                    - 'KeypointNet': Retrun KeypointNet model
    ...
    if model_name == 'TrackNet':
        ...
        model = InpaintNet()
    elif model_name == 'KeypointNet':
        model = KeypointNet(seq_len, 5, hidden_dim=64, hidden_layers=5, dropout_prob=0.2)
        # model = KeypointNet(in_channels=4, num_classes=5)
    else:
