...
class KeypointDataset(Dataset):
    def __init__(self,
        ...
        if self.frame_arr is not None:
            ...
            if rally_dir is not None:
                ...
                input_file = os.path.join(self.root_dir, f'data_l{self.seq_len}_s{self.sliding_step}_{self.split}.npz')
    ...
    def _gen_input_file(self, file_name):
        ...
        keypoint = np.array([], dtype=np.float32).reshape(0, self.seq_len)
        ...
        for rally_i, rally_dir in tqdm(self.rally_dict['i2p'].items()):
            ...
            keypoint = np.concatenate((keypoint, data_dict['keypoint']), axis=0)
        
        np.savez(file_name, id=id, frame_file=frame_file, coor=coor, vis=vis, keypoint=keypoint)
    ...
    def _gen_input_from_rally_dir(self, rally_dir):
        ...
        k = np.array(label_df['Keypoint'])
        
        # Clip based on last non-trivial keypoint
        i = min(max(np.where(k != 0)[0]) + 5, len(f_file))
        f_file = f_file[:i]
        ...
        for i in range(0, len(f_file), self.sliding_step):
            tmp_idx, tmp_frames, tmp_coor, tmp_vis, tmp_keypoint = [], [], [], [], []
            ...
            for f in range(self.seq_len):
                if i+f < len(f_file):
                    ...
                    if self.padding:
                        ...
                        tmp_keypoint.append(k[last_idx])
                    else:
                        break
            ...
            if len(tmp_frames) == self.seq_len:
                ...
                keypoint = np.concatenate((keypoint, [tmp_keypoint]), axis=0)
        return dict(id=id, frame_file=frame_file, coor=coor, vis=vis, keypoint=keypoint)
    ...
    def __getitem__(self, idx):
        ...
        if self.frame_arr is not None:
            ...
            for i in range(1, self.seq_len):
                ...
                tmp_keypoint = np.concatenate((tmp_keypoint, np.array([inter_keypoint]).reshape(1, -1), np.array([keypoint[i]]).reshape(1, -1)), axis=0)
                
                prev_coor, prev_vis, prev_keypoint = coor[i], vis[i], keypoint[i]
            
            # Resample input sequence
            rand_id = np.random.choice(len(coor), self.seq_len, replace=False)
            rand_id = np.sort(rand_id)
            tmp_coor = tmp_coor[rand_id]
            tmp_vis = tmp_vis[rand_id]
            tmp_keypoint = tmp_keypoint[rand_id]
            tmp_coor[:, 0] = tmp_coor[:, 0] / w
            tmp_coor[:, 1] = tmp_coor[:, 1] / h
            
            return data_idx, [], [], tmp_coor, tmp_vis, tmp_keypoint
            ##########
            ...
            if self.frame_alpha > 0:
                ...
                for i in range(1, self.seq_len):
                    ...
                    prev_img, prev_heatmap, prev_coor, prev_vis, prev_keypoint = cur_img, cur_heatmap, coor[i], vis[i], keypoint[i]
                ...
                frames = frames.reshape(self.seq_len + int(self.bg_mode == "concat"), 3, self.HEIGHT, self.WIDTH)
                heatmaps = heatmaps[:, np.newaxis, :, :]
                
                return data_idx, np.concatenate((frames, heatmaps), axis=1), heatmaps, tmp_coor, tmp_vis, tmp_keypoint
            else:
                ...
                keypoint = self.data_dict['keypoint'][idx]
                w, h = self.img_config['img_shape'][data_idx[0][0]]
                w_scaler, h_scaler = self.img_config['img_scaler'][data_idx[0][0]]
                ...
                return data_idx, np.concatenate((frames, heatmaps), axis=1), heatmaps, coor, vis, keypoint
